...
void receive_Data() { //need to time out after some 8 bit no response after bytes start coming in
    data_found = false;
    ...
    if (data_found == true) { //read in response
        Serial.print("RX t=");
...
byte ModRTU_CRC_check(byte message[], byte result[]) {           //returns byte array
    uint16_t crc = 0xFFFF;                  // initialize
    for (int pos = 0; pos < 8; pos++) {     // 8 is data array length so pos ranges 0 to 7 as array index
        result[pos] = message[pos];
        crc ^= (uint16_t)message[pos];      // XOR byte into least sig. byte of crc
        for (int i = 8; i != 0; i--) {      // Loop over each bit
            if ((crc & 0x0001) != 0) {      // If the LSB is set
